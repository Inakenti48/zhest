    ...
    <div className="space-y-6 sm:space-y-8 px-4 sm:px-0">
      ...
      {loading && activeTab !== 'nesting' ? (
        ...
        <>
          ...
          {activeTab === 'salary' && (
            <div className="space-y-6">
              ...
              <div className="glass p-4 sm:p-6 overflow-hidden">
                ...
                <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0 no-scrollbar">
                  <table className="full text-sm min-w-[600px]">
                    <thead>
                      <tr className="text-left text-metal-500 uppercase text-[10px] tracking-widest">
                        <th className="pb-4">Сотрудник</th>
          ...
          {activeTab === 'nesting' && (
            <div className="space-y-6">
              <div className="glass overflow-hidden relative border border-white/10 shadow-2xl rounded-3xl">
                ...
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-emerald-600/5 blur-[100px] pointer-events-none" />
                  ...
                  <div className="relative z-10 p-5 sm:p-6">
                    ...
                    <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/10">
                      <div className="p-3 bg-gradient-to-br from-blue-600 to-blue-400 rounded-xl shadow-lg shadow-blue-500/20 text-white">
                        <Scissors size={20} />
                      </div>
                      <div>
                        <h2 className="text-lg sm:text-xl font-black uppercase tracking-tight">КАЛЬКУЛЯТОР РАСКРОЯ</h2>
                        <p className="text-metal-400 text-[9px] sm:text-[10px] font-medium uppercase tracking-widest mt-0.5">Единая система расчёта изделий и стоимости</p>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-6">
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="space-y-1.5 bg-white/5 p-3.5 rounded-2xl border border-white/10 focus-within:border-blue-500/50 transition-all">
                                <div className="flex items-center justify-between">
                                  <span className="text-[9px] font-black text-metal-500 uppercase">Ширина</span>
                                ...
                                <input
                                  ...
                                  className="w-full bg-transparent border-none p-0 text-xl font-black focus:ring-0 text-white placeholder:text-white/10"
                                  placeholder="0"
                                />
                              </div>
                              <div className="space-y-1.5 bg-white/5 p-3.5 rounded-2xl border border-white/10 focus-within:border-blue-500/50 transition-all">
                                <div className="flex items-center justify-between">
                                  <span className="text-[9px] font-black text-metal-500 uppercase">Длина</span>
                                  <div className="flex gap-1 bg-black/40 p-0.5 rounded-md border border-white/5">
                                    {['cm', 'm'].map(u => (
                                      <button 
                                        key={u}
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              <div className="space-y-1.5 bg-white/5 p-3.5 rounded-2xl border border-white/10 focus-within:border-orange-500/50 transition-all">
                                <div className="flex items-center justify-between">
                                  <span className="text-[9px] font-black text-metal-500 uppercase">Ширина (W)</span>
                                  <div className="flex gap-1 bg-black/40 p-0.5 rounded-md border border-white/5">
                                    {['cm', 'm'].map(u => (
                                      <button 
                                        key={u}
                              ...
                              <div className="space-y-1.5 bg-white/5 p-3.5 rounded-2xl border border-white/10 focus-within:border-orange-500/50 transition-all">
                                <div className="flex items-center justify-between">
                                  <span className="text-[9px] font-black text-metal-500 uppercase">Длина (L)</span>
                                  <div className="flex gap-1 bg-black/40 p-0.5 rounded-md border border-white/5">
                                    {['cm', 'm'].map(u => (
                                      <button 
                                        key={u}
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="bg-white/5 p-3.5 rounded-2xl border border-white/10 focus-within:border-purple-500/50 transition-all">
                              <span className="text-[9px] font-black text-metal-500 uppercase block mb-1">Нужно изделий (шт)</span>
                              <input
                                type="number"
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-2 gap-3">
                              <div className="p-4 bg-emerald-600/10 rounded-2xl border border-emerald-500/20 transition-all shadow-sm">
                                <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-2">На 1 лист</p>
                                <p className="text-3xl font-black text-white">{nestingResults.totalParts} <span className="text-[10px] text-metal-500 uppercase">шт</span></p>
                              </div>
                              <div className="p-4 bg-purple-600/10 rounded-2xl border border-purple-500/20 transition-all shadow-sm">
                                <p className="text-[9px] font-black text-purple-500 uppercase tracking-widest mb-2">Нужно листов</p>
                                <p className="text-3xl font-black text-white">{nestingResults.sheetsNeeded} <span className="text-[10px] text-metal-500 uppercase">шт</span></p>
                              </div>
                            </div>
  
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div className="p-4 bg-red-600/5 rounded-2xl border border-red-500/10 flex items-center justify-between shadow-sm">
                              <div>
                                <p className="text-[8px] font-black text-red-500 uppercase tracking-widest mb-1">ОСТАТОК Ш</p>
                                <p className="text-xl font-black text-white">{nestingResults.wasteWidth.toFixed(1)} <span className="text-[10px] text-metal-500">см</span></p>
                              </div>
                              <div className="text-right">
                                <p className="text-[7px] font-black text-metal-500 uppercase tracking-widest">ДЛИНА ЛИСТА</p>
                                <p className="text-base font-black text-metal-400">{nestingParams.sheetLength.toFixed(1)} <span className="text-[10px]">см</span></p>
                              </div>
                            </div>
                            <div className="p-4 bg-red-600/5 rounded-2xl border border-red-500/10 flex items-center justify-between shadow-sm">
                              <div>
                                <p className="text-[8px] font-black text-red-500 uppercase tracking-widest mb-1">ОСТАТОК Д</p>
                                <p className="text-xl font-black text-white">{nestingResults.wasteLength.toFixed(1)} <span className="text-[10px] text-metal-500">см</span></p>
                              </div>
                              <div className="text-right">
                                <p className="text-[7px] font-black text-metal-500 uppercase tracking-widest">ШИРИНА ЛИСТА</p>
                                <p className="text-base font-black text-metal-400">{nestingParams.sheetWidth.toFixed(1)} <span className="text-[10px]">см</span></p>
                              </div>
                            </div>
                          </div>
  
                          <div className="p-5 bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-2xl border border-emerald-400/20 flex flex-col gap-5 shadow-xl shadow-emerald-900/40">
                            <div className="flex items-center gap-4 border-b border-white/10 pb-4">
                              <div className="p-2.5 bg-black/30 text-white rounded-xl border border-white/10 shrink-0 shadow-lg">
                                <DollarSign size={20} />
                              </div>
                              <div>
                                <p className="text-[9px] font-black text-emerald-200 uppercase tracking-widest">ИТОГО К ОПЛАТЕ</p>
                                <h3 className="text-sm font-black text-white uppercase tracking-tight leading-tight">СТОИМОСТЬ ПАРТИИ</h3>
                              </div>
                            </div>
  
                            <div className="flex items-center justify-between gap-6">
                              <div className="w-32 space-y-1 bg-black/30 p-3 rounded-xl border border-white/10 focus-within:border-white/30 transition-all shadow-inner">
                                <span className="text-[8px] font-black text-white/50 uppercase tracking-widest">Коэфф.</span>
                                <input
                                  type="number"
                                  step="any"
                                  value={extraCalc.coeff || ''}
                                  onChange={(e) => setExtraCalc({ ...extraCalc, coeff: parseFloat(e.currentTarget.value.replace(/^0+(?=\d)/, '')) || 0 })}
                                  className="w-full bg-transparent border-none p-0 text-xl font-black focus:ring-0 text-white placeholder:text-white/20"
                                  placeholder="0"
                                />
                              </div>
  
                              <div className="text-right">
                                <p className="text-3xl sm:text-4xl font-black text-white tracking-tighter tabular-nums drop-shadow-lg leading-none">
                                  {Math.round(nestingParams.partWidth * nestingParams.partLength * extraCalc.coeff * 0.01 * (nestingParams.targetParts || 1))}
                                  <span className="text-xl text-white/70 ml-2">₽</span>
                                </p>
                              </div>
                            </div>
                          </div>
